...
const USERNAME = "Penguinjaa"; // Put your LastFM username here
const BASE_URL = `https://lastfm-last-played.biancarosa.com.br/${USERNAME}/latest-song`;

const cleanSongTitle = (title) => {
    return title.replace(/\(feat.*?\)/gi, "").replace(/\(with.*?\)/gi, "").replace(/\[feat.*?\]/gi, "").replace(/\[with.*?\]/gi, "");
};
...
const getTrack = async () => {
    ...
    let trackImage = json.track.image[1]['#text'];
